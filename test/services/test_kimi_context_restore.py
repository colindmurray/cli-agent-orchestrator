...
def test_render_reads_full_compact_projection():
    ...
    held = kr.render_restoration(_projection(goal={
        "active_hold": {
            "hold_id": "h1", "state": "active",
            "reason_kind": "human-approval",
            "release_kind": "gate", "release_id": "g7",
...
def test_render_refuses_blank_objective():
    assert kr.render_restoration(_projection(
        goal={"objective": "  "})) is None
...
def test_render_ignores_legacy_requirements_field():
    ...
    text = kr.render_restoration(_projection(goal={
...
def test_render_mutations_change_sent_text():
    ...
    assert (kr.render_restoration(_projection(goal={
        "completion_requirements": [
            {"id": "changed-req", "kind": "report-occurrence"}]})) != base)
    assert (kr.render_restoration(_projection(goal={
        "latest_checkpoint": {
            "event_id": "ev-10", "kind": "checkpoint",
            "goal_version": 3, "recorded_at": "t2",
            "summary": "moved on"}})) != base)
    assert (kr.render_restoration(
        _projection(next_action="stand down")) != base)
...
def test_wrapper_hold_activation_between_reads_delivers_current_only(
    ...
    out, err = io.StringIO(), io.StringIO()
    assert kr.run_wrapper(
        hook_input={"session_id": "sess-1", "trigger": "auto",
    ...
    assert "cao-context-restoration" in kr.render_restoration(
        posted["projection"])
...
def test_post_boundary_passes_request_identity_through(monkeypatch):
    ...
    kr._post_boundary(
        ...
        request_id="req-run-1", projection=projection,
        hook_evidence=evidence, timeout_seconds=1)
    kr._post_boundary(
        fork_base="http://x", terminal_id="t", fence=fence,
        request_id="req-run-1", projection=projection,
        hook_evidence=evidence, timeout_seconds=1)
...
def test_two_runs_mint_distinct_request_ids(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        kr, "_run_conduct",
        lambda argv, timeout_seconds=None, **k: next(answers))
...
def test_wrapper_rereads_fence_under_shared_hold(tmp_path, monkeypatch):
    ...
    def fake_post(*, fork_base, terminal_id, fence, request_id,
                    projection, hook_evidence, timeout_seconds):
        events.append("post")
        posted["fence"] = fence
...
def _install_submit_world(monkeypatch, *, legs=None, transport=None):
    ...
    resolved = types.SimpleNamespace(
        ...
        execution_mode=em.NATIVE_TUI, provider_version="0.29.2",
    ...
    monkeypatch.setattr(
        _services_pkg, "control_input_service", svc, raising=False)
    monkeypatch.setattr(
        _services_pkg, "cohort_journal", journal, raising=False)
    monkeypatch.setattr(
        _services_pkg, "pane_input_arbiter", arbiter, raising=False)
    monkeypatch.setattr(
        _services_pkg, "control_input_journal", cij, raising=False)
    monkeypatch.setattr(
        _utils_pkg, "terminal", uterm, raising=False)
...
def _projection(**over):
    ...
    goal = {"goal_id": "goal-1", "goal_version": 3,
            "objective": "ship it",
            "completion_requirements": [
            ...
            "evidence_count": 2,
            "latest_checkpoint": {
                "event_id": "ev-9", "kind": "checkpoint",
                "goal_version": 3, "recorded_at": "t",
                "summary": "halfway there"},
            "active_hold": None,
            "next_action": "draft the report"}
    goal.update(over.pop("goal", {}))
    answer = {"result_type": "ok",
              "identity": {"generation_fence": "verified"},
              "goal": goal,
...
def _wire_echo(home, marker):
    ...
    wire.write_text(json.dumps({
        ...
        "message": {"role": "user",
                    "content": [{"type": "text",
                                 "text": f"x [cao-context-restoration marker:{marker}]"}],
                    "origin": {"kind": "user"}},
        "time": 5}) + "\n")
...
def test_same_event_retry_adopts_once(tmp_path, monkeypatch):
    ...
    first = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_retry_A",
...
def test_distinct_same_text_events_deliver_twice(tmp_path, monkeypatch):
    ...
    fence = _submit_fence(str(lockdir / "goal-effect.lock"))
    first = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_2x_A",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
    ...
    assert typed
    second = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_2x_B",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
...
def test_two_compactions_same_goal_delivers_new_context(tmp_path, monkeypatch):
    ...
    first = kr.submit_context_reminder(
        ...
        fence=fence, hook_evidence=_evidence(estimated_token_count=11))
    assert first["status"] == "posted", first
    assert first["new_bytes"] is True
    typed = list(box["transport"].calls)
    assert typed
    second = kr.submit_context_reminder(
        ...
        fence=fence, hook_evidence=_evidence(estimated_token_count=22))
    assert second["status"] == "posted", second
    assert second["new_bytes"] is True
...
def test_marker_free_partial_blocks_until_proven_empty(
        tmp_path, monkeypatch):
    ...
    fence = _submit_fence(str(lockdir / "goal-effect.lock"))
    first = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_pf_A",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
    ...
    # the wire is silent.
    _stub_capture(monkeypatch, rows=_full_box(
        "> truncated reminder without marker", "operator draft line"))
    second = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_pf_B",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
        hook_evidence=_evidence(estimated_token_count=42))
    assert second["status"] == "refused", second
    assert second["new_bytes"] is False
    assert (second["record"]["refusal_reason"]
            == knc.REFUSED_COMPOSER_NONEMPTY)
    assert box["transport"].calls == typed
    ...
    _stub_capture(monkeypatch, rows=_empty_box())
    third = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_pf_C",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
...
def test_unreadable_composer_refuses_only_that_session(
        tmp_path, monkeypatch):
    ...
    bad = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_un_r",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
        hook_evidence=_evidence())
    assert bad["status"] == "refused", bad
    assert bad["new_bytes"] is False
    assert (bad["record"]["refusal_reason"]
            == knc.REFUSED_COMPOSER_NONEMPTY)
    assert box["transport"].calls == []
    _stub_capture(monkeypatch, rows=_empty_box())
    good = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_un_g",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
...
def test_unpinned_build_refuses_with_pin_recovery(tmp_path, monkeypatch):
    ...
    _, box = _install_submit_world(monkeypatch)
    monkeypatch.setattr(
        kr, "_fork_lifecycle_working", lambda **k: None)
    monkeypatch.setattr(
        kr, "_fork_occurrence_current", lambda **k: None)
    ...
    fence = _submit_fence(str(lockdir / "goal-effect.lock"))
    result = kr.submit_context_reminder(
        terminal_id="t-submit", operation_id="op_up_X",
        occurrence_id="occ-1", projection=_projection(), fence=fence,
        hook_evidence=_evidence())
    assert result["status"] == "refused", result
    assert result["new_bytes"] is False
    assert (result["record"]["refusal_reason"]
            == knc.REFUSED_COMPOSER_UNPINNED)
