...
def restore_command(
    *,
    ...
    parts = [
        ...
        "--terminal",
        shlex.quote(terminal_id),
        "--terminal-generation",
        shlex.quote(generation),
        "--conduct",
        shlex.quote(conduct_binary),
    ]
    if fork_base:
...
def render_restoration(answer: Dict[str, Any]) -> Optional[str]:
    ...
    for req in (goal.get("completion_requirements") or [])[:20]:
        if isinstance(req, dict):
            shown.append(f"- {req.get('id', '?')}: {req.get('summary', req.get('kind', ''))}")
        elif isinstance(req, str) and req.strip():
            shown.append(f"- {req.strip()}")
    if shown:
        lines.append("completion requirements:\n" + "\n".join(shown))
    outstanding = [r for r in (goal.get("requirements_outstanding") or []) if isinstance(r, str)]
    if outstanding:
...
def _run_conduct(conduct_argv: List[str], *, timeout_seconds: float) -> Dict[str, Any]:
    ...
    return (
        ...
        else {"transport_error": "conduct answer is not an object"}
    )
...
def _valid_flock_path(flock_path: object) -> bool:
    ...
    return (
        isinstance(flock_path, str)
        and os.path.basename(flock_path) == "goal-effect.lock"
        and os.path.isabs(flock_path)
    )


def managed_wire_roots(*, terminal_id: str, generation: Optional[str]) -> List[str]:
    ...
    if terminal_id and generation:
        candidates.append(str(Path(COMPANION_DIR) / terminal_id / generation / "kimi-home"))
        candidates.append(str(Path(COMPANION_DIR) / "kimi-homes" / f"{terminal_id}-{generation}"))
    try:
...
def hook_evidence_from_input(hook_input: Dict[str, Any], *, observed_at: str) -> Dict[str, Any]:
...
def _post_boundary(
    *,
    ...
    return (
        body if isinstance(body, dict) else {"transport_error": "boundary answer is not an object"}
    )


def _fresh_fence_problem(
    *,
    fresh: object,
    held_flock_path: str,
    ...
    if not isinstance(fresh, dict) or "transport_error" in fresh:
        detail = fresh.get("transport_error", fresh) if isinstance(fresh, dict) else fresh
    ...
    if str(fence["flock_path"]) != held_flock_path:
        return (
            f"fresh fence moved to a different project lock "
            f"({fence['flock_path']!r}); the held lock no longer "
            f"covers it"
        )
    if (
        terminal_generation
        and fence.get("terminal_generation")
        and str(fence["terminal_generation"]) != str(terminal_generation)
    ):
        return "fresh fence generation moved under this wrapper"
    if terminal_id and fence.get("terminal_id") and str(fence["terminal_id"]) != str(terminal_id):
    ...
    if native_session_id and str(fence["native_session_id"]) != str(native_session_id):
...
def run_wrapper(
    *,
    ...
    if native_session_id and stdin_session and stdin_session != native_session_id:
        ...
        print(
            f"cao-kimi-hook-context: stdin session {stdin_session!r} does not "
            f"match baked {native_session_id!r}; restoring nothing",
            file=err,
        )
        return 0
    ...
    answer = _run_conduct(conduct_argv, timeout_seconds=CONDUCT_TIMEOUT_SECONDS)
    if not isinstance(answer, dict) or "transport_error" in answer:
        print(
            f"cao-kimi-hook-context: projection unavailable: "
            f"{answer.get('transport_error', answer)}",
            file=err,
        )
        return 0
    ...
    if not _valid_flock_path(held_flock):
        print(
            f"cao-kimi-hook-context: deferred (locator fence names no "
    ...
    from cli_agent_orchestrator.services import goal_effect_flock as _flock

    try:
        with _flock.hold_path(held_flock, shared=True, timeout_seconds=FENCE_FLOCK_TIMEOUT_SECONDS):
            fresh = _run_conduct(conduct_argv, timeout_seconds=CONDUCT_TIMEOUT_SECONDS)
            problem = _fresh_fence_problem(
                fresh=fresh,
                held_flock_path=held_flock,
                terminal_id=terminal_id,
                terminal_generation=terminal_generation,
            ...
            if problem is not None:
                print(
                    f"cao-kimi-hook-context: deferred ({problem}; retry "
            ...
            if render_restoration(fresh) is None:
                print(
                    f"cao-kimi-hook-context: deferred (fresh projection "
            ...
            evidence = hook_evidence_from_input(
                hook_input, observed_at=datetime.now(timezone.utc).isoformat()
            )
        ...
        print(
            f"cao-kimi-hook-context: deferred (project lock held: {exc}; "
            f"the next PostCompact notification or the sentinel periodic "
            f"rendezvous retries; the due clock and any pending row are "
            f"untouched; no bytes sent)",
            file=err,
        )
        return 0
    if "transport_error" in boundary:
        print(
            f"cao-kimi-hook-context: delivery boundary unreachable: "
            f"{boundary['transport_error']}",
            file=err,
        )
        return 0
    print(
        f"cao-kimi-hook-context: boundary answered " f"{boundary.get('status', boundary)}", file=err
    )
    return 0
...
def parse_wrapper_argv(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--conduct", required=True, help="conductor CLI binary (absolute path baked at install)"
    )
    parser.add_argument(
        "--fork-base",
        default=None,
...
def _fork_wait_cover(
    *, session_name: str, terminal_id: str, generation: Optional[str]
    ...
    try:
        listing = registered_waits.list_waits(session_name=session_name, terminal_id=terminal_id)
    except Exception as exc:
        return ("wait_unreadable", f"fork wait list unreadable, deferring: {exc}")
    ...
    for w in rows:
        ...
        if owner_gen is not None and generation is not None and str(owner_gen) != str(generation):
            continue
    ...
    return ("wait_cover", f"covering wait states {sorted(states)} constrain this worker")


def _fork_occurrence_current(
    *, occurrence_id: str, terminal_id: str, generation: Optional[str]
) -> Optional[Tuple[str, str]]:
    ...
    try:
        ...
        return ("occurrence_unreadable", f"fork occurrence unreadable, deferring: {exc}")
    if not isinstance(occ, dict) or occ.get("state") != "open":
        return ("occurrence_closed", f"occurrence {occurrence_id!r} is not open; discarding")
    bound = occ.get("bound_terminal_id", occ.get("terminal_id"))
    bound_gen = occ.get("bound_generation", occ.get("generation"))
    if bound != terminal_id or (
        bound_gen is not None and generation is not None and bound_gen != generation
    ):
        return (
            "occurrence_moved",
...
def _fork_lifecycle_working(*, session_name: str) -> Optional[Tuple[str, str]]:
    ...
    try:
        ...
        return ("lifecycle_unreadable", f"session lifecycle unreadable, deferring: {exc}")
    state = lifecycle.get("lifecycle") if isinstance(lifecycle, dict) else None
    if state != "working":
        return (
            "lifecycle_not_working",
...
def _observe_branch(
    *, pane_id: str, terminal_id: str, session_name: str, window_name: str
...
def _composer_empty_gate(
    *, pane_id: str, provider_version: Optional[str], viewport_rows: Optional[Sequence[str]]
) -> Optional[Tuple[str, str]]:
    ...
    if viewport_rows is None:
        return (
            adapter.REFUSED_COMPOSER_NONEMPTY,
    ...
    if pin is None:
        return (
            adapter.REFUSED_COMPOSER_UNPINNED,
    ...
    try:
        ...
        empty = pane.observe_composer_empty(pane_id, pin, screen=lambda: rows)
...
def _settle_adopted_row(record: Dict[str, Any], *, roots: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        if settlement.get("reconciled") and isinstance(settlement.get("record"), dict):
    ...
    *,
    ...
    if resolved.pane_id is None or resolved.pane_dead:
        return {
            "status": "refused",
            "new_bytes": False,
            "detail": "pane is gone or dead; nothing was typed",
        }
    ...
    try:
        ...
        return {"status": "unknown", "new_bytes": False, "detail": f"retry lookup failed: {exc}"}
    if known is not None:
        ...
        if (
            known.get("terminal_id") != terminal_id
        ...
        if adapter._intent_occurrence(known.get("intent")) not in (None, occurrence_id):
            return {
                "status": "refused",
                "new_bytes": False,
        ...
        roots = managed_wire_roots(terminal_id=terminal_id, generation=resolved.terminal_generation)
        ...
        return {
            "status": "pending",
            "new_bytes": False,
            "detail": f"retry adopted reminder {operation_id!r}; " "backing off",
            "record": record,
        }
    ...
    try:
        with goal_effect_flock.hold_path(flock_path, shared=True, timeout_seconds=10.0):
            with cohort_journal.session_effect_admission(resolved.session_name):
                ...
                if problem is not None:
                    if problem[0] == "lifecycle_not_working":
                        return _journal_fence_refusal(adapter.REFUSED_LIFECYCLE, problem[1])
                ...
                if problem is not None:
                    ...
                    if reason in ("occurrence_closed", "occurrence_moved"):
                        return _journal_fence_refusal(adapter.REFUSED_OCCURRENCE, problem[1])
                    return {"status": "deferred", "new_bytes": False, "detail": problem[1]}
                ...
                if problem is not None:
                    ...
                    if reason in ("wait_cover", "wait_recovery"):
                        return _journal_fence_refusal(adapter.REFUSED_WAIT_COVER, problem[1])
                    return {"status": "deferred", "new_bytes": False, "detail": problem[1]}
                turn_state, turn_detail = _observe_branch(
                    pane_id=resolved.pane_id,
                    terminal_id=terminal_id,
                    session_name=resolved.session_name,
                    window_name=managed_window_name(terminal_id, str(resolved.terminal_generation)),
                )
                chord = None
                ...
                with control_input_service.provider_byte_admission(
                    ...
                    with pane_input_lease(
                        ...
                        roots = managed_wire_roots(
                            terminal_id=terminal_id, generation=resolved.terminal_generation
                        )
                        ...
                        live = [r for r in rows if r.get("state") in ("posted", "accepted")]
                        owned = [r for r in rows if r.get("state") in ("intended", "writing")]
                        ...
                        if live:
                            ...
                            if target.get("state") in ("completed", "refused"):
                                return _gate_or_deliver({r.get("operation_id") for r in ambiguous})
        ...
        return {
            "status": "unknown",
            "new_bytes": False,
            "detail": f"delivery failed before journaling: {exc}",
        }
    ...
    *,
    ...
    if wrapper is None or conduct is None:
        ...
        return dict(base_environment), describe_installation(
            ...
            kimi_home=None,
            degraded_reason=(
                f"context restoration not installed ({missing} "
    ...
    return environment, describe_installation(
        terminal_id=terminal_id, generation=generation, kimi_home=home, degraded_reason=degraded
    )
