"""The Claude native control state machine, driven end to end.
...
Claude check, so the two suites never share a store, a schema, or a
fixture.
...
def _no_real_settle(monkeypatch):
    ...
    The pinned 2.0s exists because the Ink renderer swallows an Enter that
    arrives too soon; the delay is real behaviour, not test slack. It is
...
def _attach(
    *,
    ...
    """Take one Claude session all the way to attached, owned by this terminal.

    The acquisition method is the chosen-id one, because a Claude session
    id is minted here and handed to the provider — it was never read back
    out of anything.
    """
...
class TestSubmitSettle:
    def test_the_renderer_is_given_the_pinned_settle_before_the_enter(self, _no_real_settle):
        """The Ink TUI swallows an Enter that arrives too soon.
        ...
        assert _no_real_settle == [2.0]

    def test_an_unpinned_single_line_settles_at_the_proven_floor(self, _no_real_settle):
        """A missing pin never selects the zero the Ink renderer swallows.

        The floor is the longest proven interval for this provider —
        waiting longer than a build needs costs latency, while the zero
        the fallback used to select is exactly the timing documented to
        leave the message sitting unsubmitted with no error.
        """
        _attach()
...
class TestTurnObservationProvenance:
    ...
    def test_a_kimi_turn_observation_cannot_satisfy_a_claude_gate(self):
        _attach()
        foreign = dict(_idle())
...
class TestDefensiveGuards:
    ...
    def test_an_unproven_build_gets_the_floor_settle_marked_unproven(self):
        ...
        ``2.0`` on an unproven build is the longest proven interval for
        ...
        """
        _attach()
        plan = cnc.plan_composer_keystrokes("one line only", provider_version="9.9.9")
    ...
    def test_corrupt_journal_json_reads_as_absent_rather_than_raising(self):
        ...
        with database.SessionLocal() as db:
            row = db.query(database.ClaudeNativeControlOperationModel).filter_by(
                operation_id="op-64"
            )
...
class TestChosenIdentityIsAcceptedByTheAttachmentStore:
    def test_a_chosen_session_id_is_its_own_acquisition_method(self):
        """The other two describe acquiring an id the provider already had.

        A bootstrap reads one back out of a transport and a resume names
        one that exists; recording either for a freshly minted id would
        make the journaled receipt say the session pre-dated the launch.
        """
        assert na.ACQUISITION_CHOSEN_SESSION_ID in na.ACQUISITION_METHODS

        intent = na.acquire_intent(
            acquisition_method=na.ACQUISITION_CHOSEN_SESSION_ID,
            acquisition_receipt={"native_session_id": SESSION, "id_source": "cli_session_id"},
            admits_only_new_instructions=True,
            replays_task_bytes=False,
        )

        # The store accepts only an intent it built itself, so this is the
        # check that the Claude launch path can actually claim a session.
    ...
    def test_bootstrap_assertions_are_meaningless_for_a_chosen_id(self):
        """There was no bootstrap, so there is nothing to assert about one."""
        with pytest.raises(na.NativeAttachmentInvalid, match="meaningless"):
            na.acquire_intent(
                acquisition_method=na.ACQUISITION_CHOSEN_SESSION_ID,
