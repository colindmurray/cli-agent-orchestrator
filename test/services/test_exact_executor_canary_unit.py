...
def test_installed_cell_matrix_uses_the_provider_proof_gates():
    # Unpinned: any semver-shaped Codex build is runnable, because the
    # bootstrap proves its own contract against the installed binary rather
    # than looking the build up. A failed observation still is not.
    assert matrix.assess_cell("codex", normalized_version="0.147.0").runnable
    assert matrix.assess_cell("codex", normalized_version="0.145.0").runnable
    assert matrix.assess_cell("codex", normalized_version="0.999.0").runnable
    assert not matrix.assess_cell("codex", normalized_version="").runnable
    assert not matrix.assess_cell("codex", normalized_version="not-a-version").runnable
