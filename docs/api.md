...
**Response:** Terminal object (201 Created). A retry carrying the same contract on a fixed session name adopts the exact durable ready winner with zero effects: 200 OK, the stored terminal with its stored receipt, and the `x-cao-adopted: exact-ready` header — even when the profile source has since drifted, because the stored winner (receipt, ready state, request identity) is validated before the mutable source is consulted. A retry whose request (working directory, tools, env, memory-manager input) or stored receipt differs, or whose winner is not provider-ready, answers 409 with zero effects. `allowed_tools` is order-identity-bearing in the request fingerprint: `normalize_request_tools` preserves list order and `supervisor_create_request_fingerprint` hashes the ordered list (`src/cli_agent_orchestrator/services/supervisor_profile_receipt.py`), and adoption compares the stored fingerprint by exact string equality (`terminal_service.py`), so the same tool names in a different order are a different request — never sort, dedupe, or normalize the list as a set, and a retry must resend the exact frozen order. Example: an original request with `allowed_tools: ["Read", "Bash"]` is adopted only by a retry with `["Read", "Bash"]`; a retry with `["Bash", "Read"]` answers 409 (request fingerprint mismatch) with zero effects. This is request identity, not permission meaning: the effective allowed-tools policy is still enforced as a set at launch — order only decides whether a retry counts as the same request. (Contrast: `env_vars` keys sort in `canonical_env_hash`, so reordered env keys still adopt.) A sealed-capable launch carries the runtime-authored `cao-profile-receipt-v1` in `profile_receipt` — the profile, route, provenance, source path, and digest the runtime actually launched with. A legacy launch (unsupported adapter, no contract) records no exact receipt: `profile_receipt` stays null.
