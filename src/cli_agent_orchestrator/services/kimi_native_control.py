...
#: interval for this provider — never the null value: the pinned settles
#: exist because an Enter that arrives too soon is swallowed by the
#: renderer's paste-burst handling, leaving the message unsubmitted with
#: no error, so a zero fallback would reproduce exactly that failure on
#: exactly the builds nothing was verified for. Waiting longer than a
#: build needs costs latency; waiting shorter loses the message silently.
